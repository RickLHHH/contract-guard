...
export function generateMockAIReview(contractText: string): AIReview {
  ...
  const hasPaymentTerms = /付款|支付|账期/i.test(contractText);
  const hasJurisdiction = /管辖|仲裁|争议解决/i.test(contractText);
  const hasTermination = /解除|终止|违约/i.test(contractText);
  const hasConfidentiality = /保密|保密义务/i.test(contractText);
  const hasIP = /知识产权|专利|商标|著作权/i.test(contractText);
  ...
  if (penaltyMatch) {
    const penaltyPercent = penaltyMatch[1].includes('%') 
      ...
      : 30; // default if Chinese characters
  ...
  // Analyze IP
  if (!hasIP && /技术|开发|设计|创作/i.test(contractText)) {
    risks.push({
      id: 'ai-risk-5',
      clause: '合同全文',
      location: '整体',
  ...
  if (!/不可抗力/i.test(contractText)) missingClauses.push('不可抗力条款');
  ...
  return {
    ...
    keyRisks: risks,
    missingClauses,
    thinking: `经过对合同的全面审查，发现${risks.length}个主要风险点。${highRiskCount > 0 ? '其中存在' + highRiskCount + '个高风险项需要重点关注。' : ''}建议优先处理管辖条款和违约责任条款的修改。`,
